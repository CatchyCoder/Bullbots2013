...
public class Main extends IterativeRobot {
    ...
    public void robotInit() {
        ...
        pidTrackingController.setOutputRange(-0.5, 0.5);
        ...
        pidDepthController.setOutputRange(-0.7, 0.7);
    ...
    public void teleopPeriodic() {
        if(!isClimbing){
            try {
		// For left and right auto-adjusting
                if(joystick.getButton(2)) {
                    trackingController.trackGoal();
                    driveTrain.turnPID();
                }
                
		// For forward auto-adjusting
                else if(joystick.getButton(3)) {
                    trackingController.trackGoal();
                    driveTrain.forwardPID();
                }
		
		// For manual driving & shooting
		else driveTrain.driveSpeed(joystick.getYAxis(), joystick.getXAxis());
                if(gamepad.getButton(1)) shooter.shoot();
            }
            catch(Exception e){
                e.printStackTrace();
            }
